...
#define MY_NUM_HALF_PERIODS 150
/*
 * GLOBAL VARIABLES
 */
...
static int32_t myReadTemp() {
    ...
    state = ((GPIO_ReadValue(0) & (1 << 2)) != 0);

    /* get next state change before measuring time */
    while(((GPIO_ReadValue(0) & (1 << 2)) != 0) == state);
    state = !state;

    t1 = getTicks();

    for (i = 0; i < MY_NUM_HALF_PERIODS; i++) {
        while(((GPIO_ReadValue(0) & (1 << 2)) != 0) == state);
        state = !state;
    }
    ...
    return ( (2*1000*t2) / (MY_NUM_HALF_PERIODS) - 2731 );
}

static void printTemp() {
	uint32_t temp_value = myReadTemp();
	printf("%2.2f degrees\n",temp_value/10.0);
